...
%        RMSerr: estimated state-dependent localizaton errors
...
%
% ML 2017-07-31 : adapted to mleYZdXs and mleYZdXc and models
...
% the version with time-dependent errors
[est,est2]=mleYZdXt.parameterEstimate(W,varargin{:});
% add error estimates
est.RMSerr=sqrt(W.P.v); % RMS errors
if(numel(est.RMSerr)==1) % handle the case of a single error estimate
    est.RMSerr=est.RMSerr*ones(1,W.numStates);
end

if(~isempty(fields(est2)))
    
    isSlow=est2.isSlow;
    isFast=~isSlow;
    if(isempty(isSlow) || isempty(isFast) )
        warning('spt.parameterEstimate 2-state coarsegraining: D threshold outside D interval')        
        est2.RMSerr=nan(1,2);
    else
        est2.RMSerr=sqrt([est.RMSerr(isSlow).^2*rowNormalize(est.pOcc(isSlow))'  est.RMSerr(isFast).^2*rowNormalize(est.pOcc(isFast))']);
    end
end
