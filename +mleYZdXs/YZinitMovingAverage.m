function [YZ,Dest,Vest]=YZinitMovingAverage(X,wRad,dDtol,tau,R,dt)

tInit=tic;
wWidth=2*wRad-1; % width of moving average (off integer)
W=mleYZdXs.init_P_dat(tau,R,1,dt,1,1,1,X);
Dest=nan(size(X.x(:,1))); % estimated local diffusion constant
Vest=nan(size(X.x(:,1))); % estimated localization variance

% default guess, to use in place of long missing data stretches
W=mleYZdXs.converge(W,X,'display',0); % backup for stretches of missing data
...
for n=1:numel(X.i0)
    ...
    if(sum(isfinite(x0.x(:,1)))>1) % then something can meaningfully be estimated
        w=mleYZdXs.converge(w,x0,'display',0,'Nwarmup',1,'lnLrelTol',1e-6);
        for nn=1:numel(ff)
    ...
    if(Tn>wWidth) % then there is room for a moving average
        ...
        for t=wRad+1:(Tn-wRad)+1 % intermediate points
            ...
            if(sum(isfinite(x0.x(:,1)))>1) % then something can meaningfully be estimated
                ...
                for it=1:100
                    lam0=w.P.lambda;
                    w=mleYZdXs.diffusionPathUpdate(w,x0);
                    w=mleYZdXs.parameterUpdate(w,x0);
                ...
                Vest(ind(iyz(wRad:wWidth+1)))=w.P.v;
            end
        end
    end
