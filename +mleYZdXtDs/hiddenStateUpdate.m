...
% [W,sMaxP,sVit,WS]=hiddenStateUpdate(W,dat) one round of hidden state EM
% iteration (maximum likelihood) in a diffusive HMM with explicit
% detachemtn rates, with possibly missing position data
%
...
lnH=lnH-sum(...
        ...
        ,2)/2*(1./W.P.lambda);
% detatchment rate contributions
lnD=             zeros(T   ,W.numStates)+log(1-W.P.d);
lnD(W.YZ.i1-1,:)=zeros(Ntrj,W.numStates)+log(W.P.d  );
lnD(W.YZ.i1,:)=0;
lnH=lnH+lnD;
lnH(W.YZ.i1,:)=0;
lnHmax=max(lnH,[],2);
...
if(nargout>=3) % compute Viterbi path, with a small offset to avoid infinities
    ...
    error('viterbi path with detachment rates not implemented for mleYZdXtDs yet')
end
if(nargout>=4)
