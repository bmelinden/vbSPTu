...
% convergence, using a YZdXt HMM model with detachment rates d(s).
%
% Output:
...
%
% Input :
% W     : EM6 HMM model struct, e.g., created by mleYZdXtDs.init_P_dat
% dat   : EM6 data struct, e.g., from spt.preprocess
...
for r=1:(Nwarmup+maxIter)
    ...
    % iterate
    W=mleYZdXtDs.hiddenStateUpdate(W,dat);
    dlnLrel=(W.lnL-lnL0)/abs(W.lnL);
    lnL0=W.lnL;
    W=mleYZdXtDs.diffusionPathUpdate(W,dat);
        
    if(r>Nwarmup)
        if(Pupdate)
            W=mleYZdXtDs.parameterUpdate(W,dat);
        end
        if(exist('lam0','var'))
...
if(nargout>=2) % compute sequence of most likely states
    [W,sMaxP]=mleYZdXtDs.hiddenStateUpdate(W,dat);
end
if(nargout>=3) % compute Viterbi path, with a small offset to avoid infinities
    [W,sMaxP,sVit]=mleYZdXtDs.hiddenStateUpdate(W,dat);
end
