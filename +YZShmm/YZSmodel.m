...
methods (Access = private)
    function new = copy(this)
        % Make a copy of the model object, but do not duplicate data.
        new = vbXdXmodel([],this.opt,this.numStates);
    ...
    function [w1,dat1,ind]=reorder(this,dat,ind)
        % [w1,dat1,ind]=reorder(this,dat,ind)
        % create a vbXdXmodel and data struct with trajectory order
        % given by the index vector ind (default: resampled with
        % replacement for bootstrap)
        Ntrj=numel(this.YZ.i0);
        if(~exist('ind','var'))
            ind=sort(ceil(Ntrj*rand(1,Ntrj)));
        end
        dat1=vbspt.reorderTrj(dat,ind);
        w1=vbXdXmodel(dat1,this.opt,this.numStates);
        w1.P=this.P;
        fnames={'muY','muZ','varY','varZ','sigYYp1','sigYZ','sigYp1Z'};
        for k=1:numel(ind)
            %indOld=this.YZ.i0(ind(k)):this.YZ.i1(ind(k))-1;
            %indNew=new.YZ.i0(k):new.YZ.i1(k)-1;
            %new.S.pst(indNew,:)=this.S.pst(indOld,:);
            
            indOld=this.YZ.i0(ind(k)):this.YZ.i1(ind(k));
            indNew=w1.YZ.i0(k):w1.YZ.i1(k);
            for f=1:numel(fnames)
                w1.YZ.(fnames{f})(indNew,:)=this.YZ.(fnames{f})(indOld,:);
            end
        end
        % recompute hidden states rather than explicitly reorder them
        w1.Siter();
    end
    function compress(this)
        % vbXdXmodel.compress()
        % Reduce the storage footprint of a XdXmodel object by removing some bulky
        % internal variables. After loading a compressed variable, the missing
        % parts can be recreated by the uncompress method.
        
        this.convergence=[];
        
        % decide which is bulkiest, S or YZ, and delete the bulkiest
        nColsYZ =7*size(this.YZ.muY,2);
        nColsPST=this.numStates;
        if(nColsYZ >= nColsPST)
            this.YZ.muY=[];%
            this.YZ.muZ=[];%: [5050x2 double]
            this.YZ.varY=[];%: [5050x2 double]
            this.YZ.varZ=[];%: [5050x2 double]
            this.YZ.sigYYp1=[];%: [5050x2 double]
            this.YZ.sigYZ=[];%: [5050x2 double]
            this.YZ.sigYp1Z=[];%: [5050x2 double]
        else
            this.S.pst=[];
        end
        %% clear interal copy of trajectory data
        %if(nargout>0)
        %    dat=this.dat;
        %end
        %this.dat=[];
    end
    function uncompress(this,varargin)
        % XdXmodel.uncompress(...)
        % Expand a XdXmodel object that has been compress()ed. This requires the
        % dataset used to create the model (or one compatible with it), which can
        % be done in three different ways (W is the XdXobject) :
        %
        % W.uncompress('opt0'),  uses the internal options struct to reload the data
        % W.uncompress('opt',opt), uses a supplied options struct opt instead (but
        %       does not change the existing internal options struct).
        % W.uncompress('dat',X), uses a vbspt data struct X.
        
        % recreate data
        if(numel(varargin)~=2)
           error('uncompress requires a parameter,value-pair as input')
        end
        param=varargin{1};
        switch lower(param)
            case 'opt0'
                dat=vbspt.preprocess(this.opt);
            case 'opt'
                dat=vbspt.preprocess(varargin{2});
            case 'dat'
                dat=varargin{2};
            otherwise
                error(['vbXdXmodel.uncompress : parameter ' param ' not recognized'])
        end
        
        % recreate empty fields
        if(isempty(this.S.pst))
            this.Siter();
        else
            this.YZiter(dat);
        end
    end
    function P=propsToStruct(this)
        % P=propsToStruct(this)
        % return all properties of the object in the fornm of a struct.
        % More memory efficient?
        
        fname=properties(this);
        P=struct();
        for k=1:numel(fname)            
            P.(fname{k})=this.(fname{k});
        end
        
    end
    function M=meanPar(this)
        % M=vbXdXmodel.meanPar()
        % compute variational posterior mean parameter values
        M=struct;
        M.numStates=this.numStates;
        M.D= this.P.c./(this.P.n-1)/2/this.timestep;
        M.p0=this.P.wPi/sum(this.P.wPi);
        M.pOcc=rowNormalize(sum(this.S.pst,1));
        M.tTot=sum(this.S.pst,1);
        a=rowNormalize(this.P.wa);
        M.A = diag(a(:,2))+diag(a(:,1))*rowNormalize(this.P.wB);
        M.dwellSteps = 1./a(:,1)';
        M.dwellTime= M.dwellSteps*this.timestep;
        M.wA=this.S.wA;
        M.F=this.F;
    end
    function s=sViterbi(this)
        % most likely sequence of states, extended to s(T+1)=s(T), from
        % using the Viterbi algorithm on the variational hidden state
        % distribution.
        s=this.Siter();
        s(this.YZ.i1)=s(this.YZ.i1-1);
    end
    function s=sMaxP(this)
       % sequence of most likely states, extended to s(T+1)=s(T), using the
       % variational hidden state distribution.
       [~,s]=max(this.S.pst,[],2);    
       s(this.YZ.i1)=s(this.YZ.i1-1);
    end
    function D=Dmean(this)
       % state-averaged diffusion constant <D(t)>, extended to
       % <D(T+1)>=<D(T)>, using the variational parameter and hidden state
       % distributions.
        D=this.S.pst*this.meanPar.D';
        D(this.YZ.i1)=D(this.YZ.i1-1);
    end
    function Y=toCells(this,X)
        % split a vector reprepresentation into a cell vector, where the
        % cells have the same size as the input trajectories.
        Y=cell(1,numel(this.YZ.i0));
       for k=1:numel(Y)
          Y{k}=X(this.YZ.i0(k):this.YZ.i1(k)-1,:);
       end
    end
    function [t,X]=removeTp1(this,X)
