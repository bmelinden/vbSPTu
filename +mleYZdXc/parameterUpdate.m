...
% A parameter update iteration (MLE) on a diffusive YZdXc HMM (constnat
% localization errors), including handling missing data 
...
ot(W.i1)=false; % should be redundant

nv=0.5*W.dim*sum(ot);
cv=0.5*sum(sum((dat.x(ot,:)-W.YZ.muZ(ot,:)).^2+W.YZ.varZt(ot,:)));
W.P.v=cv/nv;
